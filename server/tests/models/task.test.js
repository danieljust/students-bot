...
describe('task model tests', () => {
  it('should return compare deadlines', () => {
    return taskService.create({deadline: moment("12-25-1995", "MM-DD-YYYY"), description: 'Awesome_description'})
      .then(task => taskService.findOneById(task._id))
      .then(task => {
        expect(moment("12-25-1995", "MM-DD-YYYY").isSame(task.deadline)).toBeTruthy();
      });
  });
  it('should return same description', () => {
    return taskService.create({deadline: moment("12-25-1995", "MM-DD-YYYY"), description: 'Awesome_description'})
      .then(task => taskService.findOneById(task._id))
      .then(task => {
